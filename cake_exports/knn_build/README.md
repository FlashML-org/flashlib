...
```bash
cd cake_exports/knn_build/generated
python build.py --arch sm_100a --domains knn_build
python -m knn_build.demo
```
...
`knn_build.interface`; its tensor and scalar ABI is documented by
`TENSOR_KEYS` and `SCALAR_NAMES` in that module.

## Benchmark contract

The recorded denominator is the complete 112-shape Cake KNN-build contract.
The baseline is `flashlib.flash_knn` from this repository's main commit
...
```bash
cd /path/to/cake
python /path/to/flashlib/cake_exports/knn_build/benchmarks/benchmark.py \
  --domain knn_build \
  --output-root /tmp/knn-build-results \
  --reuse-export-root /path/to/flashlib/cake_exports/knn_build/generated \
