...
class KNNBuildRuntime:
    ...
    def compute(
        ...
        *,
        ...
        pairs, so consecutive default-output calls never alias).
        """
    ...
    def clear(self, *, synchronize: bool = True) -> None:
        ...
        every plan-held launch argument, slot-owned norm buffer, and pooled
        default output is tied to its plan's stream via ``record_stream``
        before release, keeping tensor storage allocator-safe; the caller
        ...
        with self._lifecycle:
            ...
            try:
                ...
                with self._cache_lock:
                    if synchronize:
                        # Device work is complete; release the driver graph
                        # handles eagerly. A non-synchronizing clear only
                        # drops the Python references — an executing graph
                        # must never be destroyed underneath the device.
