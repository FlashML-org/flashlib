...
Provenance: loom @ ff502f39df09ffdb317efc57ebdac3a668bb3aa4. k-NN search (top-K nearest neighbours); builds random inputs, runs, checks against a torch brute force.
...
def run_shape(shape, *, device="cuda", seed=0):
    ...
    if ok and norm_sources:
        ...
        torch.cuda.synchronize()
        ok, detail = _check(named, shape, reference)
        print(f"[knn_search] {'OK  ' if ok else 'FAIL'} {label} route={route} internal-norms {detail}")
    return ok
