...
// Provenance: loom @ 7a7cebd73ee298188e1ed3b6d6d8d7e43dbf5f04. schedule symbol 'flash_kmeans_assign_gapfused_highd_splitk_partial_oob_v1'; embedded in the host TU as flash_kmeans_assign_gapfused_highd_splitk_partial_oob_v1_b8e3ecee40.
...
kernel_flash_kmeans_assign_gapfused_highd_splitk_partial_oob_v1(const void* __restrict__ x_tmap, const void* __restrict__ c_tmap, float* __restrict__ c_sq, float* __restrict__ partial_scores, int* __restrict__ partial_indices, int B, int N, int D, int K, int num_n_tiles, int K_tiles)
{
                ...
                if (elect_sync()) {
                    ...
                    for (unsigned int work_idx = bid; work_idx < total_work; work_idx += num_bids) {
                        ...
                        for (int feat_tile = 0; feat_tile < feature_tiles; feat_tile++) {
                            int feat_col = feat_tile * 64;
                            mbarrier_wait(x_empty_addr, _phase_x_empty_0);
                            _phase_x_empty_0 ^= 1;
                            tma_2d_gmem2smem(smem_x_addr, x_tmap, feat_col, x_row, x_full_addr);
                            mbarrier_arrive_expect_tx(x_full_addr, 16384);
                            mbarrier_wait(c_empty_addr, _phase_c_empty_0);
                            _phase_c_empty_0 ^= 1;
                            tma_2d_gmem2smem(smem_c_addr, c_tmap, feat_col, c_row, c_full_addr);
