...
// Provenance: loom @ unknown. schedule symbol 'flash_kmeans_assign_d112_consumer_scoped_direct_mma_view_handoff_e5e1_v1'; embedded in the host TU as flash_kmeans_assign_d112_consumer_scoped_direct_mma_view_handoff_e5e1_v1_6fe708594c.
...
kernel_flash_kmeans_assign_d112_consumer_scoped_direct_mma_view_handoff_e5e1_v1(__nv_bfloat16* __restrict__ x, __nv_bfloat16* __restrict__ centroids, float* __restrict__ c_sq, int* __restrict__ out, int B, int N, int D, int K, int num_n_tiles)
{
    ...
    for (unsigned int tile = cluster_id; tile < total_tiles; tile += num_clusters) {
        ...
        for (unsigned int i = tid; i < 7168; i += 256) {
            ...
            int col = i % 112;
            {
                __nv_bfloat16 _bval_3705015744 = __float2bfloat16_rn(x_raw[i]);
                uint16_t _bits_3705015744 = *(uint16_t*)&_bval_3705015744;
                uint32_t _addr_3705015744 = static_cast<uint32_t>((sx_addr + (unsigned int)(row * 224 + col * 2)));
                asm volatile("st.shared.b16 [%0], %1;" :: "r"(_addr_3705015744), "h"(_bits_3705015744) : "memory");
            }
        }
        ...
        for (int group_kt = 0; group_kt < group_k_tiles; group_kt++) {
            ...
            for (int rp = 0; rp < 2; rp++) {
                ...
                for (int cp = 0; cp < 2; cp++) {
                    ...
                    int cc = lane % 4 * 2 + cp;
                    {
                        uint32_t _addr_3705015888 = static_cast<uint32_t>((ss_addr + (unsigned int)((group * 64 + row_base + rr) * 32 + cc * 4)));
                        asm volatile("st.shared.f32 [%0], %1;" :: "r"(_addr_3705015888), "f"(acc[rp * 2 + cp]) : "memory");
                    }
                }
            }
            __syncwarp();
