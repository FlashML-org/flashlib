...
// Provenance: loom @ 9dd3df73eea8632ec145151e6ae5c5a88dd59969. schedule symbol 'flash_kmeans_assign_gapfused_paired_oob_v1'; embedded in the host TU as flash_kmeans_assign_gapfused_paired_oob_v1_b8e3ecee40.
...
kernel_flash_kmeans_assign_gapfused_paired_oob_v1(const void* __restrict__ x_tmap, const void* __restrict__ c_tmap, float* __restrict__ x_sq, float* __restrict__ c_sq, int* __restrict__ out, int B, int N, int D, int K, int num_n_tiles, int K_tiles)
{
                ...
                if (elect_sync()) {
                    ...
                    for (unsigned int tile_idx = bid; tile_idx < num_tiles; tile_idx += num_bids) {
                        ...
                        for (int chunk = 0; chunk < 2; chunk++) {
                            tma_2d_gmem2smem(smem_x0_addr + (unsigned int)(chunk * 128 * 128), x_tmap, chunk * 64, x_row0, x0_full_addr);
                        }
                        ...
                        for (int chunk_1 = 0; chunk_1 < 2; chunk_1++) {
                            tma_2d_gmem2smem(smem_x1_addr + (unsigned int)(chunk_1 * 128 * 128), x_tmap, chunk_1 * 64, x_row1, x1_full_addr);
                        }
                        ...
                        for (int iter_k = 0; iter_k < K_tiles; iter_k++) {
                            ...
                            for (int chunk_2 = 0; chunk_2 < 2; chunk_2++) {
                                tma_2d_gmem2smem(smem_c_addr + (unsigned int)(chunk_2 * 256 * 128), c_tmap, chunk_2 * 64, c_row, c_full_addr);
                            }
